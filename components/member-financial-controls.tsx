...
function BankCard({ type, application, onApply, pending }: { type: "debit" | "credit"; application?: CardApplication; onApply: () => void; pending: boolean }) {
  ...
  return <div className="overflow-hidden rounded-xl border bg-card shadow-sm">
    ...
    <div className="space-y-4 p-4">
      ...
      <div className="grid grid-cols-2 gap-2 border-y py-3 text-xs"><div><p className="text-muted-foreground">{isCredit ? "Annual fee" : "Monthly fee"}</p><p className="mt-1 font-medium">$0</p></div><div><p className="text-muted-foreground">{isCredit ? "Credit limit" : "Access"}</p><p className="mt-1 font-medium">{isCredit ? (application?.creditLimit ? formatCents(application.creditLimit) : "Set after approval") : "Free ATM access"}</p></div></div>
...
export function MemberFinancialControls({ recipients, email }: { recipients: Recipient[]; email: string }) {
