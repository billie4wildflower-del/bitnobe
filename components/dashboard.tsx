    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        ...
        {activeView === "overview" && <div className="grid gap-4 lg:grid-cols-3">
          ...
          <Card><CardHeader><CardTitle className="text-sm text-muted-foreground">Account standing</CardTitle></CardHeader><CardContent className="space-y-4"><div className="flex items-center gap-3"><CheckCircle2 className={`h-5 w-5 ${account.status === "active" ? "text-emerald-600" : "text-amber-600"}`} /><div><p className="font-medium capitalize">{account.status}</p><p className="text-xs text-muted-foreground">{account.status === "active" && !account.fraudFreeze ? "No restrictions" : "Some account actions may be limited"}</p></div></div><div className="flex items-center gap-3"><ShieldCheck className={`h-5 w-5 ${account.fraudFreeze ? "text-amber-600" : "text-emerald-600"}`} /><div><p className="font-medium">{account.fraudFreeze ? "Under review" : "Protected"}</p><p className="text-xs text-muted-foreground">BitNobe monitored</p></div></div></CardContent></Card>
        ...
        {activeView === "accounts" && <div className="space-y-4"><div className="grid gap-4 md:grid-cols-2">
        ...
        </div><MemberFinancialControls recipients={recipients} email={user.email} /></div>}

        {activeView === "transfers" && <div className="space-y-4"><div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]"><Card><CardHeader><CardTitle>Internal transfers</CardTitle><CardDescription>Find a registered member by email or account number.</CardDescription></CardHeader><CardContent><Button onClick={() => setSendOpen(true)}><Send className="mr-2 h-4 w-4" />Start a transfer</Button><p className="mt-4 text-sm text-muted-foreground">Recent recipients are suggested as you type. You can save any recipient to your contacts.</p></CardContent></Card><Card><CardHeader><CardTitle>Transfer policy</CardTitle></CardHeader><CardContent className="space-y-3 text-sm text-muted-foreground"><p>Verify the member and account number before sending.</p><p>Available balance is checked again when the transfer is posted.</p><p>For a reversal or investigation, contact BitNobe Support.</p></CardContent></Card></div><MemberFinancialControls recipients={recipients} email={user.email} /></div>}
