  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        ...
        {activeView === "transfers" && <div className="space-y-4"><div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]"><Card><CardHeader><CardTitle>Internal transfers</CardTitle><CardDescription>Find a registered member by email or account number.</CardDescription></CardHeader><CardContent><Button onClick={() => setSendOpen(true)}><Send className="mr-2 h-4 w-4" />Start a transfer</Button><p className="mt-4 text-sm text-muted-foreground">Recent recipients are suggested as you type. You can save any recipient to your contacts.</p></CardContent></Card><Card><CardHeader><CardTitle>Transfer policy</CardTitle></CardHeader><CardContent className="space-y-3 text-sm text-muted-foreground"><p>Verify the member and account number before sending.</p><p>Available balance is checked again when the transfer is posted.</p><p>For a reversal or investigation, contact BitNobe Support.</p></CardContent></Card></div><MemberFinancialControls recipients={recipients} /></div>}
