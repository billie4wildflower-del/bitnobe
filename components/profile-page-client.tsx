      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        ...
        <div className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
          ...
          <div className="space-y-4">
            <Card><CardHeader><CardTitle className="flex items-center gap-2"><LockKeyhole className="h-5 w-5 text-primary" />Security status</CardTitle><CardDescription>Protections active on your member access.</CardDescription></CardHeader><CardContent className="space-y-3 text-sm"><div className="flex items-center justify-between border-b pb-3"><span>Password sign-in</span><span className="font-medium text-emerald-600">Enabled</span></div><div className="flex items-center justify-between"><span>Account access</span><span className="font-medium text-emerald-600">Active</span></div><p className="pt-2 text-xs text-muted-foreground">For a password reset or account concern, contact BitNobe Support.</p></CardContent></Card>
            <Card><CardHeader><CardTitle>Bank account</CardTitle><CardDescription>Your primary BitNobe checking account.</CardDescription></CardHeader><CardContent className="space-y-4 text-sm"><div className="flex justify-between"><span className="text-muted-foreground">Available balance</span><span className="font-semibold">{formatCents(profile.account.balance)}</span></div><div className="flex justify-between"><span className="text-muted-foreground">Account number</span><span className="font-mono">{maskAccount(profile.account.accountNumber)}</span></div><div className="flex justify-between"><span className="text-muted-foreground">Routing number</span><span className="font-mono">{profile.account.routingNumber}</span></div><p className="border-t pt-3 text-xs text-muted-foreground">Opened {new Date(profile.account.openedAt).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</p></CardContent></Card>
          </div>
        </div>
      </main>
    </div>
