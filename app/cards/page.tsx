...
export default async function CardsPage() {
  ...
  try {
    ...
    return <div className="min-h-screen bg-background"><TopNav name={summary.user.name} email={summary.user.email} activeView="accounts" isAdmin={summary.isAdmin} /><main className="mx-auto max-w-6xl px-4 py-6 sm:px-6"><div className="mb-6"><p className="text-sm text-muted-foreground">Member services</p><h1 className="text-3xl font-semibold tracking-tight">Cards & payment access</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Enroll your debit card or apply for a credit card with a reviewed credit limit. Wire withdrawals and transfer contacts remain in Accounts and Transfers.</p></div><MemberFinancialControls recipients={recipients} email={summary.user.email} /></main></div>
  } catch {
